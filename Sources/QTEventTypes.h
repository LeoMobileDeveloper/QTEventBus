...
/**
 ...
 举个例子：[QTEventBus shared].on(QTJsonEvent.class).ofType(@"Login").ofType(@"Logout")
 
 表示同时监听QTJsonEvent下面的id为Login和Logout事件
 */
- (QTEventSubscriberMaker<Value> *)ofType:(NSString *)eventType;
...
@property (readonly, nonatomic) QTEventSubscriberMaker<Value> *(^ofType)(NSString *);
